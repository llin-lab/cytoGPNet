...
class simple_VAE(nn.Module):
    def __init__(self, input_dim, embed_dim=2, hidden_dim=4):
        """
        simple VAE structure (share the same encoder for different marker clusters by PAD) (\theta_1: VAE parameters).
        :param input_dim: the dimensionality of the input cells for each marker cluster
        :param hidden_dim: VAE hidden layer dim.
        :param embed_dim: marker embedding dim.
        """
        ...
        self.decoder = nn.Sequential(nn.Linear(embed_dim, hidden_dim),
...
class simple_AE(nn.Module):
    def __init__(self, input_dim, embed_dim=2, hidden_dim=4):
        """
        simple AE structure (share the same encoder for different marker clusters by PAD) (\theta_1: AE parameters).
        :param input_dim: the dimensionality of the input cells for each marker cluster
        :param hidden_dim: AE hidden layer dim.
        :param embed_dim: marker embedding dim.
        """
