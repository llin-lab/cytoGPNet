...
class simple_VAE(nn.Module):
    def __init__(self, input_dim, embed_dim=2, hidden_dim=4):
        """
        simple VAE structure.
        :param input_dim: input dimensionality (n_markers)
        :param hidden_dim: hidden layer size
        :param embed_dim: latent/embedding size
        """
        ...
        self.fc1 = nn.Linear(hidden_dim, embed_dim)  # mean
        self.fc2 = nn.Linear(hidden_dim, embed_dim)  # log variance
...
class GaussianProcessLayer(gpytorch.models.ApproximateGP):
    def __init__(self, input_dim, num_inducing_points, inducing_points, mean_inducing_points,
                 grid_bounds, likelihood_type, using_ngd, using_ksi, using_ciq, using_sor, using_OrthogonallyDecouple):
        """
        Define the mean and kernel function: Constant mean, and additive kernels (each kernel for a marker cluster).
        ...
        :param input_dim: input embedding dim.
        :param inducing_points: inducing points at the latent space (n, k*input embedding dim).
    ...
    def forward(self, x):
        ...
        covar = self.covar_module(x) # Checked this part, covar = self.each_kernel(x)
...
class Attention_Layer(nn.Module):
    def __init__(self):
        super().__init__()
        self.w = nn.Linear(1, 1, bias=False)  # Scalar weight on cell level

    def forward(self, X):
        """
        X: tensor of shape (batch, cell_num)
        Returns:
            output: tensor of shape (batch,) — scalar summary per sample
        """
        X_unsqueezed = X.unsqueeze(-1)  # (batch, cell_num, 1)
        scores = self.w(X_unsqueezed).squeeze(-1) * X  # (batch, cell_num)
        attn_weights = F.softmax(scores, dim=1)  # (batch, cell_num)
        output = torch.sum(X * attn_weights, dim=1)  # (batch,)
        return output
...
class Simple_Classifier(nn.Module):
    ...
    def forward(self, x):
        # Check if input is a single value (nz == 1), or else pass through the network
        if x.size(1) == 1:  # Checking input feature dimension (nz)
            return torch.sigmoid(x)  # Return sigmoid of the input if nz == 1
        else:
...
class AdditiveRBFKernel(nn.Module):
    """
    Addition of several RBF kernel functions
    RBF kernel:
